...
<img onclick="window.location.href='details'"
     src="<?php
        $email = strtolower(trim($Userinfo['user']['email'] ?? ''));
        $hash  = md5($email);
        $src   = 'https://avatar.zrahh.com/avatar/' . $hash;

        if (preg_match('/^(\d+)@qq\.com$/i', $email, $m)) {
          $src = 'https://q2.qlogo.cn/headimg_dl?dst_uin=' . $m[1] . '&spec=100';
        }
        echo $src;
     ?>"
     onerror="this.onerror=null;this.src='/themes/clientarea/yunyoo/assets/images/user.png';"
     class="ml-2 mr-2 d-flex align-items-center justify-content-center user-center_header link"
     data-avatar />
